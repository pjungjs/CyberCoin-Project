...
async function coinLiveData() {
    ...
    .then(result => {
        ...
        if(caption && tableHead && tableBody) {
...
async function history(userCoinId, userTime) {
    ...
    .then(result => {
        ...
        for (let i=0; i < result.data.length; i++) {
            ...
            coinSearchTime.push(time);
            coinSearchPrice.push(priceUsd);
        }
        createChart(coinSearchTime, coinSearchPrice);
    })
    .catch((error) => console.log(error));
}


async function createChart(coinSearchTime, coinSearchPrice) {
    const arrSetData = [];
    for (let i = 0; i < coinSearchTime.length && i < coinSearchPrice.length; i++) {
        arrSetData.push({ time: coinSearchTime[i], value: coinSearchPrice[i] });
    }
    // console.log(arrSetData)
    const canvas = document.getElementById("myChart");
    const chart = LightweightCharts.createChart(canvas, { width: 1000, height: 600 });
    const lineSeries = chart.addLineSeries();
    await lineSeries.setData(arrSetData);
}
